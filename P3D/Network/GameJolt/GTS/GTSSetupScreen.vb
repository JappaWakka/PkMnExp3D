Namespace GameJolt
    ...
    Public Class GTSSetupScreen
        ...
        Private Sub DrawStringC(ByVal t As String, ByVal p As Vector2)
			Core.SpriteBatch.DrawString(FontManager.MainFontBlack, t, p, Color.White)
		End Sub
		...
		Public Overrides Sub Draw()
			...
			If Me.IsCurrentScreen() = True Then
				Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Setup", New Vector2(132, 100), Color.White)
			...
			If Working = False And Received = Lines Then
				If Me.IsCurrentScreen() = True Then
					...
					If Selected > -1 Then
						...
						If D.TradeID <> "" Then
							Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Transaction ID: " & D.TradeID, New Vector2(510, 166), Color.Black)
						End If
						...
						Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Request:" & Environment.NewLine & Environment.NewLine &
						...
						If Core.Player.IsGameJoltSave = True Then
							Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "From:", New Vector2(516, 320), Color.Black)
							...
							Core.SpriteBatch.DrawString(FontManager.MainFontBlack, ownEmblem.Username & " (" & ownEmblem.GameJoltID & ")", New Vector2(600, 320), Color.Black)
						End If

						'To:
						Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "To:", New Vector2(816, 320), Color.Black)
						If SetupList(Selected).SecurityArea = GTSDataItem.SecurityCode.Private Then
							If Not Emblem Is Nothing Then
								If Emblem.DoneLoading = True Then
									...
									Core.SpriteBatch.DrawString(FontManager.MainFontBlack, Emblem.Username & " (" & Emblem.GameJoltID & ")", New Vector2(890, 320), Color.Black)
								Else
									Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Loading" & LoadingDots.Dots, New Vector2(864, 320), Color.Black)
								End If
							Else
								Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Loading" & LoadingDots.Dots, New Vector2(864, 320), Color.Black)
							End If
						Else
							Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Global", New Vector2(864, 320), Color.Black)
						End If
		...
		Private Sub DrawButton(ByVal Position As Vector2, ByVal Text As String, ByVal Size As Integer)
			...
			Core.SpriteBatch.DrawString(FontManager.MainFontBlack, Text, New Vector2(CSng(Position.X + sizeX / 2 - TextSizeX / 2 - 2), Position.Y + 4), Color.Black)
		End Sub
		...
		Class GTSEditTradeScreen
			...
			Public Overrides Sub Draw()
				...
				If Uploading = True Then
					...
					Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Uploading" & LoadingDots.Dots, New Vector2(CSng(Core.windowSize.Width / 2 - FontManager.MainFontBlack.MeasureString("Uploading").X / 2), 300), Color.White)
					If AssignedTradeID = True Then
						Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Transaction ID: " & NewTradeID, New Vector2(CSng(Core.windowSize.Width / 2 - FontManager.MainFontBlack.MeasureString("Transaction ID: " & NewTradeID).X / 2), 370), Color.White)
					End If
				...
				If Not D.Pokemon Is Nothing Then
					...
					If Not D.Pokemon.Item Is Nothing Then
						ItemString = "   " & D.Pokemon.Item.Name
						Core.SpriteBatch.Draw(D.Pokemon.Item.Texture, New Rectangle(124, 475, 48, 48), Color.White)
					End If
				...
				If D.TradeID <> "" Then
					Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "Transaction ID: " & D.TradeID, New Vector2(925, 102), Color.Black)
				End If
				...
				If StringHelper.IsNumeric(D.RequestID) = True Then
					If CInt(D.RequestID) > 0 Then
						...
						'        Basic.SpriteBatch.DrawString(Basic.MainFontBlack, "Needed stars:", New Vector2(780, 280), Color.Black)
						...
						'        If PaidStars > 0 Then
						'            Basic.SpriteBatch.DrawString(Basic.MainFontBlack, "Paid stars:", New Vector2(780, 380), Color.Black)

						'            GTSMainScreen.DrawStars(PaidStars, New Vector2(780, 414))

						'            Basic.SpriteBatch.DrawString(Basic.MainFontBlack, "Additional stars:", New Vector2(970, 380), Color.Black)
						...
						'        Basic.SpriteBatch.DrawString(Basic.MainFontBlack, "Your stars:", New Vector2(970, 280), Color.Black)
					...
					If NewTrade = True Then
						If IsPresent() = True Then
							If Not ToEmblem Is Nothing And Not D.Pokemon Is Nothing Then
								Core.SpriteBatch.DrawString(FontManager.MainFontBlack, "You will send " & D.Pokemon.GetDisplayName() & " to " & Environment.NewLine & ToEmblem.Username & " and you cannot get it back!", New Vector2(480, 495), Color.Black)
							End If
				...
				Else
					...
					End If
					Core.SpriteBatch.DrawString(FontManager.MainFontBlack, message, New Vector2(430, 545), Color.Black)
				End If
			...
			Private Sub DrawFilter(ByVal Position As Vector2, ByVal Size As Integer, ByVal Label As String, ByVal Text As String)
				...
				Core.SpriteBatch.DrawString(FontManager.MainFontBlack, Label, New Vector2(Position.X + 4, Position.Y + 4), Color.White)
				Core.SpriteBatch.DrawString(FontManager.MainFontBlack, Text, New Vector2(Position.X + 4, Position.Y + 32), Color.Black)
			End Sub
