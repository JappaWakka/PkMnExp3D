Public Class ChatScreen
    ...
    Public Overrides Sub Update()
		...
		If FontManager.MainFontWhite.MeasureString(Me.currentText).X > Core.windowSize.Width - 200 Then
			While FontManager.MainFontWhite.MeasureString(Me.currentText).X > Core.windowSize.Width - 200
		...
		ElseIf inputText.Length > 1 Then
			...
			For i = 0 To currentText.Length - 1
				If i <= Me.currentText.Length - 1 Then
					If FontManager.MainFontWhite.Characters.Contains(currentText(i)) = False Then
	...
	Public Overrides Sub Draw()
				...
				If i <= 13 And items <= 13 Then
					...
					If takeIndex >= 0 And takeIndex < transferedLines.Count Then
						...
						For Each l As String In lineArr
							...
							Core.SpriteBatch.DrawString(FontManager.MainFontWhite, l, New Vector2(100, (Core.windowSize.Height - 80) - offset - 64), c, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)
							offset += 32
							items += 1
						Next
						lines += 1
		...
		If Me.SelectionLength > 0 Then
			Dim startX As Integer = CInt(FontManager.MainFontBlack.MeasureString(Me.currentText.Substring(0, SelectionStart)).X)
			Dim length As Integer = CInt(FontManager.MainFontBlack.MeasureString(Me.currentText.Substring(Me.SelectionStart, Me.SelectionLength)).X)

			Canvas.DrawRectangle(New Rectangle(100 + startX, Core.windowSize.Height - 82, length, 32), Color.White)
			Core.SpriteBatch.DrawString(FontManager.MainFontBlack, Me.currentText.Substring(Me.SelectionStart, Me.SelectionLength), New Vector2(100 + startX, Core.windowSize.Height - 80), Color.White, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)
		End If
		...
		Core.SpriteBatch.DrawString(FontManager.MainFontWhite, s, New Vector2(100, Core.windowSize.Height - 80), Color.White, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)
		Dim startXSelection As Integer = CInt(FontManager.MainFontWhite.MeasureString(Me.currentText.Substring(0, Selection)).X)
		If Me.Selection = Me.SelectionStart And Me.SelectionLength > 0 Then
			startXSelection -= 2
		End If
		Core.SpriteBatch.DrawString(FontManager.MainFontWhite, "|", New Vector2(98 + startXSelection, Core.windowSize.Height - 80), Color.White, 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)
...
#End Region
	...
	Private Shared Function GetSplitIndicies(ByVal line As String) As List(Of Integer)
		Dim lineLength As Integer = CInt(FontManager.MainFontWhite.MeasureString(line).X)
			...
			For i = 0 To newMessages.Count - 1
				If newMessages.Count - 1 >= i And (newMessages.Count - 1) - i <= 10 Then
					...
					For Each l As String In lineArr
						...
						Core.SpriteBatch.DrawString(FontManager.MainFontWhite, l, New Vector2(100, (Core.windowSize.Height - 80) - offset - 64), New Color(c.R, c.G, c.B, CInt(opacity * 1.7)), 0.0F, Vector2.Zero, 1.0F, SpriteEffects.None, 0.0F)
						offset += 32
                    Next
                End If
            Next
